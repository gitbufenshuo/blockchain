...
func init() {
	...
	// createblockchainCmd.PersistentFlags().String("foo", "", "A help for foo")

	// Cobra supports local flags which will only run when this command
	// is called directly, e.g.:
	// createblockchainCmd.Flags().BoolP("toggle", "t", false, "Help message for toggle")
}
func Createblockchain(cmd *cobra.Command, args []string) {
	if len(args) == 0 {
		os.Exit(1)
	}
	fmt.Printf("creating block chain , the genesis address is %v\n", args[0])
	blockchain.CreateBlockchain(args[0])
}
